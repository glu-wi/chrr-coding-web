...
We will first calculate percentage of rural population in WI using blocks data, and then extebd the procedure to find % rural for all counties in 50 states and DC
...
We will directly download the WI blocks shapefile (277 MB) using `GeoPandas` from Census website. It would take a few minutes. (~3 min on my machine)
...
The shapefile has more information than we actually need. So let's just keep the 4 columns we need. 
```
...
Next, we will join the three population dateframes together and calculate the rural rate.
...
Let's chech the first few rows of `us_ct_rural_rate`
...
Now we can join Census data with the data from our calculations using `statecode` and `countycode` as key. In the joined data, we add a colunm `rate_diff` to see the difference between the two rates.
